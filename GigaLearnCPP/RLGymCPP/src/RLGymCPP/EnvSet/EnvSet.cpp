...
void RLGC::EnvSet::StepSecondHalf(const IList& actionIndices, bool async) {
	...
	auto fnStepArenas = [&](int arenaIdx) {
		...
		// Update rewards
		{
			...
			for (size_t rewardIdx = 0; rewardIdx < rewards[arenaIdx].size(); rewardIdx++) {
				...
				if (config.saveRewards) {
					...
					if (config.shuffleRewardSampling) {
						playerSampleIndex = Math::RandInt(0, static_cast<int>(output.size()));
					} else {
					// Find player with the lowest id
					playerSampleIndex = 0;
					uint32_t lowestID = gs.players[0].carId;
					for (size_t i = 1; i < gs.players.size(); i++) {
						uint32_t id = gs.players[i].carId;
						if (id < lowestID) {
							lowestID = id;
							playerSampleIndex = static_cast<int>(i);
						}
					}
				}
				...
				for (size_t i = 0; i < gs.players.size(); i++)
					state.rewards[playerStartIdx + static_cast<int>(i)] = allRewards[i];
		}

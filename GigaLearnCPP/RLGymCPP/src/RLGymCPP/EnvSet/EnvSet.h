...
namespace RLGC {
	...
	struct EnvCreateResult {
		...
		std::function<void(void*)> userInfoDeleter = NULL; // Optional deleter for userInfo
	};
	...
	struct EnvSet {
		...
		std::vector<std::function<void(void*)>> userInfoDeleters;
		...
		~EnvSet() {
			...
			for (int i = 0; i < userInfos.size(); i++) {
				if (userInfos[i] && userInfoDeleters[i])
					userInfoDeleters[i](userInfos[i]);
			}

			for (auto& rewardList : rewards) {
				for (auto& weighted : rewardList)
					delete weighted.reward;
			}
			for (auto& conditionList : terminalConditions) {

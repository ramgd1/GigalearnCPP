...
void GGL::GAE::Compute(torch::Tensor rews, torch::Tensor terminals,
                       torch::Tensor valPreds, torch::Tensor truncValPreds,
                       torch::Tensor &outAdvantages,
                       torch::Tensor &outTargetValues,
                       torch::Tensor &outReturns, float &outRewClipPortion,
                       float gamma, float lambda, float returnStd,
                       float clipRange, GGL::TensorPool *pool) {
  ...
  for (int step = numReturns - 1; step >= 0; step--) {
    ...
    if (terminal == RLGC::TerminalType::TRUNCATED) {
      ...
      if (!hasTruncValPreds)
        RG_ERR_CLOSE("GAE encountered a truncated terminal, but has no "
                     "truncated val pred");

      if (truncCount >= numTruncs)
        RG_ERR_CLOSE("GAE encountered too many truncated terminals, not enough "
                     "val preds (max: "
                     << numTruncs << ")")
  ...
  if (hasTruncValPreds)
    if (truncCount != truncValPreds.size(0))
      RG_ERR_CLOSE("GAE didn't receive expected truncation count (only "
                   << truncCount << "/" << truncValPreds.size(0) << ")");

  bool pinMemory = rews.is_pinned();
  outReturns = MakeHostTensorFromVec(_outReturns, {numReturns}, torch::kFloat32,
                                     pinMemory, pool);
  outAdvantages = MakeHostTensorFromVec(_outAdvantages, {numReturns},
                                        torch::kFloat32, pinMemory, pool);

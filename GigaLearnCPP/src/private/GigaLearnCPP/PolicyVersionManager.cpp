...
GGL::PolicyVersionManager::PolicyVersionManager(
    ...
    const SkillTrackerConfig &skillTrackerConfig,
    const RLGC::EnvSetConfig &envSetConfig, RenderSender *renderSender)
    : saveFolder(saveFolder), maxVersions(maxVersions),
      tsPerVersion(tsPerVersion), renderSender(renderSender) {
...
GGL::PolicyVersion &
GGL::PolicyVersionManager::AddVersion(const ModelSet &modelsToClone,
                                      uint64_t timesteps) {
  RG_NO_GRAD;

  auto models = modelsToClone.CloneAll();

  auto newVersion = PolicyVersion{timesteps, std::move(models)};
...
void GGL::PolicyVersionManager::LoadVersions(const ModelSet &modelsTemplate,
                                             int64_t curTimesteps) {
  ...
  for (int64_t savedTimesteps : allSavedTimesteps) {
    ...
    if (savedTimesteps > curTimesteps) {
      RG_ERR_CLOSE("Tried to load saved policy version that is newer than our "
                   "current model ("
                   << savedTimesteps << " > " << curTimesteps << ")!\n"
                   << "If you deleted some checkpoints, make sure to delete "
                      "that far back in the saved policy versions as well");
    }
...
void GGL::PolicyVersionManager::RunSkillMatches(PPOLearner *ppo,
                                                Report &report) {
  RG_NO_GRAD;

  auto fnUpdateRatings = [this](SkillRating &winner, SkillRating &loser,
  ...
  torch::Tensor tNewPlayers = torch::tensor(newPlayers),
  ...
  float stepTime =
      skill.envSet->config.tickSkip * RLGC::CommonValues::TICK_TIME;
  for (float t = 0;
       t < skill.config.simTime && totalSimTime < skill.config.maxSimTime &&
       skill.curGoals < skill.envSet->arenas.size();
    ...
    torch::Tensor tActionMasks =
        DIMLIST2_TO_TENSOR<uint8_t>(skill.envSet->state.actionMasks);
    ...
    PPOLearner::InferActionsFromModels(
        ...
        tNewActionMasks.to(ppo->device, true), skill.config.deterministic,
        ppo->config.policyTemperature, ppo->config.useHalfPrecision,
        &tNewActions, &_tLogProbs);
    PPOLearner::InferActionsFromModels(
        oldVersion.models, tOldStates.to(ppo->device, true),
        tOldActionMasks.to(ppo->device, true), skill.config.deterministic,
        ppo->config.policyTemperature, ppo->config.useHalfPrecision,
  ...
  for (auto &pair : skill.curRatings.data) {
    float prevRating =
        prevCurRatings.GetRating(pair.first, skill.config.initialRating);
  ...
  if (skill.curGoals < skill.envSet->arenas.size() &&
      totalSimTime < skill.config.maxSimTime) {
    // Not enough goals were scored, we will force a continuation where the same
    // models keep playing from the end position This COULD switch versions, but
    // it wouldn't be a big deal as it would just be the immediate next version
    RG_LOG(" > Forcing continuation (" << skill.curGoals << "/"
                                       << skill.envSet->arenas.size() << ")");
...
void GGL::PolicyVersionManager::OnIteration(PPOLearner *ppo, Report &report,
                                            int64_t totalTimesteps,
                                            int64_t prevTotalTimesteps) {
  // Ensure we always have at least one version
  if (versions.empty()) {
    auto models = ppo->GetPolicyModels();
    AddVersion(models, totalTimesteps);
  }

  // Save policy versions on cadence
  if (tsPerVersion > 0) {
    if (totalTimesteps / tsPerVersion > prevTotalTimesteps / tsPerVersion) {
      auto models = ppo->GetPolicyModels();
      AddVersion(models, totalTimesteps);
    }
  }
  ...
  if (skill.config.enabled) {
    ...
    if (skill.iterationsSinceRan >= skill.config.updateInterval) {
...
void GGL::PolicyVersionManager::AddRunningStatsToJSON(nlohmann::json &json) {
  ...
  json["skill_iterations_since_ran"] = skill.iterationsSinceRan;
  json["skill_cur_goals"] = skill.curGoals;
  json["skill_do_continuation"] = skill.doContinuation;
  json["skill_prev_old_version"] = skill.prevOldVersionIndex;
  json["skill_prev_new_team"] = (int)skill.prevNewTeam;
  json["skill_prev_sim_time"] = skill.prevSimTime;
}

void GGL::PolicyVersionManager::LoadRunningStatsFromJSON(
  ...
  if (json.contains("skill_iterations_since_ran"))
    skill.iterationsSinceRan = json["skill_iterations_since_ran"];
  if (json.contains("skill_cur_goals"))
    skill.curGoals = json["skill_cur_goals"];
  if (json.contains("skill_do_continuation"))
    skill.doContinuation = json["skill_do_continuation"];
  if (json.contains("skill_prev_old_version"))
    skill.prevOldVersionIndex = json["skill_prev_old_version"];
  if (json.contains("skill_prev_new_team"))
    skill.prevNewTeam = (Team)(int)json["skill_prev_new_team"];
  if (json.contains("skill_prev_sim_time"))
    skill.prevSimTime = json["skill_prev_sim_time"];
}

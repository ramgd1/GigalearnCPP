...
struct PPOLearnerConfig {
  ...
  // This will only happen if the amount of remaining experience is <
  // batchSize*2.
  bool overbatching = true;

  double maxEpisodeDuration = 120; // In seconds

  // Actions with the highest probability are always chosen, instead of being
  // more likely This will make your bot play better (usually), but is horrible
  // for learning Trying to run a PPO learn iteration with deterministic mode
  ...
  //	meaning the entropy of the state is limited to the fraction of available
  ...
  float rewardClipRange =
      10; // Clip range for normalized rewards, set 0 to disable

  bool useGuidingPolicy = false;
  std::filesystem::path guidingPolicyPath =
      "guiding_policy/"; // Path of the guiding policy model(s)
  float guidingStrength = 0.03f;

  int optimSaveInterval =
      5; // Only save optimizer every N iterations to save disk space/IO time
